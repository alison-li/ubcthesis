...
One common information source \rev{for rationale identified by} LaToza \etal is colleagues. 
However, the practice of seeking knowledgeable colleagues is challenging in \rev{many projects given turnover in a development team}. 
...
project history after a colleague’s departure to understand developer intent behind code \rev{written by a now} departed colleague wrote \cite{robillard_turnover-induced_2021}. 
...
The \rev{motivating example} suggests the developer would benefit from an automatic 
and more streamlined approach to determining non-trivial commits to \rev{consider from a file's software revision history}.
Additionally, the ability to glean the Jira issue information could help the developer 
more efficiently decide which commits \rev{to} prioritize \rev{when exploring software revision history to understand} 
...
This thesis investigates the design of a \rev{tool we name Intelligent History, 
...
We develop a set of heuristics on code changes and Jira issues that \rev{can support} a developer 
...
The design of the tool \rev{supports experimentation about} how effective the heuristics are 
...
less commits than participants who did not use Intelligent History \rev{to successfully answer}
