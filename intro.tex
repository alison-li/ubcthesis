...
There is a danger in relying solely on individuals for knowledge \rev{about} intent behind the code in a large software system.
Confronted with turnover-induced knowledge loss, Robillard \rev{finds that} developers leverage artifacts from \rev{the} 
...
Developers investigate commit history to understand ``why is this [code] ‘this’ way'' \rev{because} 
...
A typical practice in software development is to explicitly reference an issue ID from an \rev{issue tracking system} 
...
There are many tools that exist to explore commit history and manage the use of \rev{a version control system}. 
For example, ``\code{git blame}'' \cite{gitblame}, \rev{and} graphical user interfaces such as Sourcetree \cite{sourcetree} and GitExtensions \cite{gitextensions}. 
...
visit and read through the referenced issue, and maintain a mental model or narrative \rev{that} 
...
\begin{figure}
	...
	\end{RaggedRight}
	\end{mdframed}
	\caption{The commit history, shown by each commit message's subject, for \rev{the} \class{Topology} class in the open-source Apache Kafka project. 
...
\noindent which refactors several other class\rev{es} related to the Streams \entity{API} 
...
and \rev{could clutter} the commit history that a developer skims when searching for code rationale related to present code.
...
by \rev{indicating this and giving the developer the choice of skipping} these commits.
...
This results in a temporal cost of determining \rev{which} non-trivial commits to discard during exploration 
...
and more streamlined approach to determining \rev{which} non-trivial commits to consider from a file's software revision history.
...
the developer would be able to efficiently piece together the following \rev{information} about the \class{Topology} class. 
...
Since these are refactoring commits, a tool could help to optimize a developer's search by mitigating the need to visit the referenced Jira issues to \rev{discover this}.
\rev{A tool could provide the developer with the ability} to glimpse the Jira issue descriptions 
...
The Jira description leads further to a link \rev{about} open questions on topic creation and configuration in light of many requests to add the feature.
Similarly, \jira{KAFKA-7523} commited \rev{thereafter} describes another topology configuration-related shortcoming and adds support for state stores to each processor node in a topology.
These commits \rev{identify} limitations in the Kafka Streams \entity{API} and \rev{reveal insight about} user desire for more configuration support regarding topologies in Kafka. 
\rev{In turn, the discovery of this information allows} a present developer viewing the history \rev{to understand} a historical concern surrounding the \class{Topology} class.

We hypothesize \rev{that} identifying important commits automatically and providing integrated access between commit 
and issue information could help a developer more effectively access the rationale for code \rev{change}.
...
or discussions from previous developers who might have foreseen future problems related to \rev{an} existing implementation in the file.
...
\rev{fewer} commits than participants who did not use Intelligent History to successfully answer
