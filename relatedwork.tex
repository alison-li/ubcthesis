...
the intent behind a piece of code \rev{is} a serious problem, 
...
Existing \rev{Git} commands as tools for exploring revision history such 
...
\rev{In fact,} 58\% of survey respondents from the study by Codoban \etal report\rev{ed}
that understanding the evolution of code \rev{is} a primary reason for
...
\rev{Thus,} the provision of more strategic ways for viewing, filtering, and 
...
Thus, we propose to operate at the file-level, \rev{meaning we} scope the determination of
a commit as non-essential in terms of how the commit \rev{impacts} a \rev{specific file and the evolution of the file}.
