...
the commit history exploration approach they used, and their preferred aspects of Intelligent History is displayed in \autoref{fig:Flow-Chart-Individual}.
We also present an aggregated version in \autoref{fig:Flow-Chart-Aggregate} and a tabular form in \autoref{tab:Flow-Chart-Tabular}.
\FIXME{Sankey diagram for aggregate version?}
...
\begin{figure}[h]
  \includegraphics[width=12cm]{./images/flow-chart-aggr.png}
  \caption{
    TODO: Sankey diagram instead?
  }
...
\begin{landscape}
  \begin{figure}
    ...
    \begin{tabular}{@{}c|ccc@{}}
      ...
      B                       & Often                                                                                           & Cyclic and Backtracking                                                                & \begin{tabular}[c]{@{}c@{}}(A) Highlight Important Commits\\ (C) Show Jira Metadata\end{tabular} \\ \midrule
      ...
      G                       & Sometimes                                                                                       & Cyclic and Backtracking                                                                & \begin{tabular}[c]{@{}c@{}}(C) Show Jira Metadata\\ (A) Highlight Important Commits\end{tabular} \\ \midrule
      H                       & Rarely                                                                                          & Linear                                                                                 & \begin{tabular}[c]{@{}c@{}}(A) Highlight Important Commits\\ (C) Show Jira Metadata\end{tabular} \\ \midrule
      I                       & Sometimes                                                                                       & Linear                                                                                 & (C) Show Jira Metadata                                                                           \\ \midrule
      J                       & Rarely                                                                                          & Commit Message Skimming                                                                & (A) Highlight Important Commits                                                                  \\ \bottomrule
    \end{tabular}
...
Participants who vocalized familiarity with using \entity{git} ``blame'' were permitted to use the native ``Show History for Selection'' feature in IntelliJ to narrow down the commits to look at.
...
For question B(b), the failure to reject the null hypothesis may be attributed to the fact that the commit \commit{e09d6d79} that contains the information necessary for answering this question has the commit message subject: 

\begin{center}
  \jira{KAFKA-7027:}{Add overloaded build method to StreamsBuilder (\#5437)} 
\end{center}

Hence, participants locating the commit that introduced the overloaded \code{build} method would be able to quickly narrow down the commit
...
To answer question A(b), the participant would need to find a commit that introduced further \code{addSink} method overloads later in the commit history as participants were informed that the initial commit in the commit history for the \class{Topology} class introduced a set of overloaded \code{addSink} methods to begin with.
Hence, participants were prompted or intuited to locate a particular commit that introduced further overloads later in the history to understand why additional  overloads were introduced to the \class{Topology} class.
...
The other participants attempted to differentiate which \code{addSink} methods in class were not introduced as part of the initial overloads.
They then used IntelliJ's ``Show History for Selection'' feature on an \code{addSink} overloaded method that was added later to try to find the commit that introduced one.

Similarly, question B(a) necessitates for the participant to examine commits and diffs in the \class{StreamsBuilder} commit history to try identify meaningful source code changes that were made to the class.
Given the larger size of the \class{StreamsBuilder} commit history at 45 commits, this question lead to more varying strategies.
