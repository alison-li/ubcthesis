...
Moreover, Apache Kafka also has a practice of maintaining Kafka Improvement Proposals (\entity{KIP}s),
...
The first part of the laboratory session consists of 
...
Visiting the referenced Jira issue \issue{KAFKA-5670} yields no further insight due to the issue description being blank.
...
We divided the participants into two groups such that one group received 
...
For B1, both participants \participant{B} and \participant{H} did not use Intelligent History.
...
the commit history exploration approach they exhibited when searching for the answers to the question sets, 
...
We determined the feature of Intelligent History that a participant found most favourable from
...
For questions that participants completed without Intelligent History,
...
several participants commented on how commit highlighting de-emphasized ``minor'' changes from the history \participants{BDEFH}.
...
4 out of 5 participants demonstrated the most enthusiasm for (\feature{3}) \textit{Show Jira Metadata} instead \participants{ACGI},
...
to feature (\feature{3}) enabling participants to view a larger mean number of issues for question B1 than the 
...
Participants generally commented on the utility of commit history highlighting to focus their attention on commits that were highlighted, 
and ignore non-highlighted commits, which they regarded as ``irrelevant'' or ``minor'' \participants{BF}.
...
\begin{quote}
  ...
  It’s [the (\feature{1}) \textit{Highlight Important Changes} feature] really handy; it filters a bunch of -- not garbage -- but noise in the commit history. 
...
Of the 6 participants who expressed a preference for feature (\feature{1}), 
...
Participants who vocalized familiarity with and typical usage of ``\code{git blame}'' were shown and permitted to use the native 
...
and contained the rationale information by reading the commit message subjects and using the chronological information based on other commit timestamps \participants{ABDG}.
...
We anticipated question B1 would require the most application switching between the \entity{IDE} 
...
\begin{table}
  \caption{
    The categorization of each participant's qualitative responses and their demonstrated commit history exploration approach.
...
and be introduced to the Apache Kafka project, Jira, the IntelliJ IDEA \entity{IDE}, and the Intelligent History plugin,
...
perceived lack of utility from issues in their day-to-day work, then the participant might not be inclined to use Jira
