...
\begin{itemize}[leftmargin=*]
    ...
    \rev{and (\feature{2}) \textit{Show Diff Metadata}}.
...
\rev{\autoref{tab:t-test} shows the results of the} $t$-tests for determining significant statistical differences in 
...
\FIXME{Can this whole section move later? Does not seem relevant to RQ3.}
...
The flowchart \FIXME{Figure needs more explanation of rarely, \etc} showing the relationship between each of these categories for each participant
...
\FIXME{Would a whisker diagram showing the range be useful here?}
...
\rev{reported to often or sometimes} looking at commits and issues.
...
Hence, we find that the primary question responsible for \rev{participants favouring (\feature{3})} is question B1.
