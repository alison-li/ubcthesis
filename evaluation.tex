...
We chose to use Apache Kafka because the project's contribution guidelines \rev{mandate} that
...
in Apache Kafka \rev{and} involve asking the participants to find code rationale information.

The first part of \rev{the} laboratory session consists of 
...
Visiting the referenced Jira issue \issue{KAFKA-5670} yields no further insight due to \rev{the} issue description being blank.
...
We \rev{divided} the participants into two groups such that one group received 
...
Prior to commencing the experiment, we \rev{informed} the participants \rev{that} we encourage them to discuss their approach 
\rev{out loud} as to how they plan to find the information they need to answer the questions.
...
However, if a participant \rev{struggled or became confused with} how to obtain the 
information needed to answer a question, the observer would remind the participant about 
the sources of information at their disposal or \rev{would} prompt the participant to verbalize
...
We opened the interview \rev{by} asking about the participant's years of professional and non-professional software
...
and \rev{collected} the participant's opinions and feedback on using Intelligent History.
...
we \rev{asked} follow-up questions based on the participant's responses, \rev{to obtain} further clarifications 
and to probe further \rev{when there were} unexpected responses \cite{shull_guide_2007}. 
...
We \rev{designed} the study to have an estimated total duration of 1 to 1.5 hours for each participant,
...
we instrumented the Intelligent History plugin to log timestamps and events \rev{occurring} within the IDE
...
and \rev{to visualize} the impact of Intelligent History on their exploration.
...
We \rev{provide} a sample of two commit history exploration graphs to illustrate 
participants \participant{B} and \participant{H}’s exploration to answer question B1 
from \autoref{tab:Question-Sets} in \autoref{fig:Exploration-Graphs}.
For B1, both \rev{participants} \participant{B} and \participant{H} did not use Intelligent History.
Nodes in the graph are represented by commit hashes and Jira issue IDs, 
while edges express the participant's movement \rev{when going from} one commit or issue to another.
...
we categorize the graph as such regardless of whether the graph meets the \rev{criteria} for the other two categories.
...
then to \commit{88d4bc46} and back to \commit{908a6d2a}.
...
relationship between these categories for each \rev{participant}.
...
\begin{landscape}
  \begin{table}
    \caption{
      Summary of results for participants who received question set A first to complete without Intelligent History (\entity{IH}) and question set B after to complete with \entity{IH}.
      ...
      the number of application switches between the \entity{IDE} and the browser (\#AS) that the participant made,
...
\begin{landscape}
  \begin{table}
    \caption{
      Summary of results for participants who received question set B first to complete without Intelligent History (\entity{IH}) and question set A after to complete with \entity{IH}.
      The same columns shown in \autoref{tab:Results-Quantitative-AB} are used.
    }
    \centering
    \resizebox{\columnwidth}{!}{%
      \begin{tabular}{@{}crrrrrrrrrrrrrrrr@{}}
...
\rev{For} questions that participants completed without Intelligent History,
...
This indicates that in cases where Intelligent History is not used and therefore does not influence the commits \rev{that} participants examine,
...
For A1, a mean of 100.0\% of the commits \rev{examined by} participants who used Intelligent History examined 
...
Similarly for B1 and B2, a mean of 100.0\% of the commits \rev{examined by} participants who used Intelligent History 
...
While this is not sufficient evidence to \rev{conclude that participants agreed with the choice of} commits \rev{that} Intelligent History
\rev{chose} to highlight and not highlight, we observe that participants did not choose to examine the 
...
several participants commented on how commit \rev{highlighting} de-emphasized ``minor'' changes from the history \participants{BDEFH}.
...
patterns for identifying \rev{unimportant} changes in diffs show some promise as, on average,
...
We observe that a possible explanation for the outcome of the $t$-tests on A2 and B1 \rev{is}
...
Hence, participants answering questions with Intelligent History may have examined \rev{fewer} commits
\rev{because} participants used highlighted commits as a way of prioritizing which commits to investigate further.
...
could have been useful for the question \rev{where} they did not use Intelligent History.

However, among participants who used Intelligent History to answer question set B,
4 out of 5 \rev{participants} demonstrated the most enthusiasm for (\feature{3}) \textit{Show Jira Metadata} instead \participants{ACGI},
while 1 participant preferred (\feature{3}) \textit{Show Jira Metadata} \participants{E}.
For the majority of participants who used Intelligent History with set B, this result may be due
to feature (\feature{3}) enabling participants to view \rev{a} larger mean number of issues for question B1 than the 
group who did not use Intelligent History for the same question (3.2 and 2).
\rev{In particular, we expect question B1} to require accessing more Jira issues in order to be able
...
\rev{Thus, } the majority of participants who used Intelligent History
...
Participants generally commented on the utility of commit history highlighting \rev{to focus} their attention on commits that were highlighted, 
and \rev{ignore} non-highlighted commits, which they regarded as ``irrelevant'' or ``minor'' \participants{BF}.
...
participants commented \rev{that} commit history highlighting \rev{made} commit history exploration faster 
by reducing the amount of irrelevant commits they would have examined \participants{DFHJ}.
...
We \rev{defined} a meaningful source code change to be a commit that improves some functionality 
...
Despite \rev{the fact that} the group of participants who used Intelligent History to answer B2 \rev{looked} at \rev{fewer} than half the 
proportion of commits that the group who did not use Intelligent History looked at (15.6\% compared to 33.3\%),
only 1 participant from the former group emphasized commit history highlighting as a feature that made the strongest impression on them \participants{E}, 
...
Of the 6 participants \rev{who} expressed a preference for feature (\feature{1}), 
...
that operates similarly to ``\code{git blame}.''
Participants who vocalized familiarity with \rev{and typical usage of} ``\code{git blame}'' were shown and permitted to use the native 
...
and contained the rationale information by reading the commit \rev{message} subjects and using the chronological information based on other commit timestamps \participants{ABDG}.
...
in \rev{everyday} work would be able to quickly narrow down the commit needed to answer 
...
We anticipated question B1 \rev{would} require the most application switching between the \entity{IDE} 
...
\begin{table}
  \caption{
    The categorization of each \rev{participant's} qualitative responses and their demonstrated commit history exploration approach.
...
the commit history exploration strategy they exhibited as determined \rev{by} their commit history exploration graphs, 
...
\begin{summary}[RQ3]
  ...
  in contextualizing source code changes and providing access to the information in \rev{a larger number of} issues.
...
\rev{and} be introduced to the Apache Kafka project, Jira, the IntelliJ IDEA \entity{IDE}, and the Intelligent History plugin,
we restricted the number of questions asked to account for a reasonable total estimated session time.
We formulated small sets of questions to relieve participants from time \rev{pressure} 
...
asked or performed in the context of \rev{tasks} with a more concrete objective, 
...
\rev{for} finding the information necessary to answer the questions.
...
and the utility of Intelligent History \rev{for} a particular question set 
...
\rev{For example, if a participant was not accustomed to using an issue tracking system due to factors such as
...
We \rev{attempted} to minimize the impact of a participant's background related 
to looking at \rev{commits} and issues by formulating questions 
that would require participants to justify their answers 
using source code and \rev{citing specific evidence from} Jira issues.
\rev{It would be difficult for participants to surmise the rationale
