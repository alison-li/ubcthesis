...
Our current implementation requires a commit's diff to have at least \rev{one} uncategorized change
for the commit to be highlighted in the commit history.
\rev{Otherwise}, the commit is faded out from the history if it only contains trivial changes according to the regular expression patterns
...
Our \rev{tool, Intelligent History,} assumes a project uses Jira for tracking and managing issues and has
...
issue tracking systems (\entity{ITS}) beyond Jira.
...
from an \entity{ITS}, despite their organization using an \entity{ITS} with the project \participants{AH}.
...
participant \participant{E} raised a point about the discussion in an issue as difficult to 
\rev{follow and indicated that they typically looked at the source code instead
