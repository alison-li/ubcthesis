...
a change was made and only examine commits \rev{in} the date range.
...
The need to navigate \rev{numerous} commits in a revision history 
makes \rev{it more challenging to: locate} information about how a change affected a code segment, 
\rev{identify} the broader modification \rev{that the} change was part of, 
and \rev{indicate} how the code segment interacted with surrounding code at a specific point in time.
...
We convey the outcome of this analysis visually to the developer by \rev{highlighting} a file's revision history, 
...
Diffs that solely contain \rev{unimportant} changes, such as white spaces and line-endings, are inefficient 
...
and \rev{are} a significant source of noise in commit histories \cite{herzig_tangled_2013}.
Tangled changes refer to a set of changes for two or more separate tasks. 
Murphy-Hill \etal \cite{murphy-hill_refactor_2012} found refactorings are often committed with \rev{combined} changes for separate tasks. 
From a study of five open-source Java projects, Herzig and Zeller \cite{herzig_tangled_2013} 
revealed \rev{that} up to 15\% of all bug fixes consist of tangled changes and, 
...
The issues in issue repositories \rev{are} a valuable for obtaining rationale information 
as \rev{they} contain past discussions about design aspects \cite{brunet_design_2014,hassan_road_2008}.
Issues from an issue tracking system (\entity{ITS}) typically contain a title, 
...
and opinions \rev{about} the ``why'' of certain design choices in those changes \cite{ortu_jira_2015}.
...
\begin{figure}
    ...
    \caption{
        ...
        rev{This} illustrates the distributed nature of code change and rationale information between the \entity{IDE} and browser.
    }
...
We propose comput\rev{ing} a subset of information from issues as metadata and \rev{then displaying the} relevant metadata
alongside issues to the developer.
\rev{This information and presentation} could potentially signal to a developer whether a commit's associated issue
...
This is because the information \rev{that} Intelligent History offers is integrated within the \entity{IDE}, 
...
This makes the features of Intelligent History \rev{complementary} to IntelliJ's existing version control interface.
...
\begin{itemize}
    ...
    \item[(\feature{3})] \textit{Show Jira Issue}: 
        ...
        this action also provides a metadata summary of the Jira issue containing metrics that could indicate an issue as likely to contain rationale information \rev{for} an
        ...
        the total number of comments on the Jira issue \rev{excluding} bot comments, 
        the total number of unique people involved in the Jira issue based on comments including the Jira issue assignee, 
        the number of people subscribed to the Jira issue (``watches''), the number of votes on the Jira issue, the number \rev{of} issue links, 
...
\begin{figure}
    ...
    \caption{
        ...
        \rev{This} shows the explicitly referenced Jira issue's title, assignee, reporter, description, priority, 
...
and \class{StreamsBuilder} are available in a public repository.\footnote{\url{https://github.com/Alison-Li/ubcthesis-dataset}, verified 7/25/2022.}
...
\begin{itemize}
    ...
    \item \textit{Newline}: Changes involving the insertion or deletion of newlines. We consider these changes as \rev{cosmetic} and unlikely to yield any further insight.
...
inserting an \code{import} statement, and \rev{modifying} a Javadoc comment.
...
The Jira issues open \rev{as tabs} in a separate tool window in IntelliJ to allow the developer to view several issues at a time and compare them.
...
Among these 26 commits that reference \rev{a} Jira issue, the developer wishes to locate commits that contain information about the motivation and design choices involved in the changes they see in the diff.
When a commit message is \rev{insufficient} for providing information about the rationale behind a commit, the developer invokes (\feature{3}).
On the first 4 highlighted commits in the commit history, the developer is able to skip perform\rev{ing} an in-depth investigation into the Jira issues for commits with empty or un-informative Jira issue descriptions since the developer can know this beforehand using without leaving the \entity{IDE} by invoking feature (\feature{3}) on each highlighted commit.
...
making it harder for consumers of the \entity{API} who use a modern \entity{IDE} to discover the interfaces and hence, \rev{it} was a usability problem.
...
associated with the commit can further enrich a commit's message and diff without further explanation \rev{about} the motivation and rationale behind a commit's changes.
Overall, the features of Intelligent History reduce the time it \rev{takes} for a developer 
in this example to manually determine relevant commits, and \rev{the features} minimize the switches the developer \rev{needs} 
