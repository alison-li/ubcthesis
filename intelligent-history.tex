...
The design of our approach in \toolname{Intelligent History} is guided by two motivating principles: 
...
Given that the filtering functionality based on commit messages is already an available feature in existing version control \entity{gui} applications and in IntelliJ IDEA's native Git interface, we propose to use a technique that automatically reduces the search space of commits using the diffs for each commit.
...
Consequently, in implementing \toolname{Intelligent History}, we have chosen to automate the detection of certain categories of code changes we consider to be wasteful for a developer's time when traversing the commit history for a single file.
