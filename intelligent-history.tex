...
The issues in issue repositories is a valuable source that current members of a project can use for obtaining rationale information as it contains past discussions about design \cite{brunet_design_2014,hassan_road_2008}.
...
\autoref{fig:IntelliJ-Overview} shows IntelliJ with a file's commit history visible \FIXME{It would be good to label the figure}.
...
\begin{enumerate}[label=(\Alph*)]
    ...
    \item \textit{Show Diff Metadata}: \FIXME{This description should motivate why such a summary would be helpful.} An action that displays a pop-up summary of metadata information on the diff for a user-selected commit in a file's commit history. This summary contains a categorization of code changes in a commit's diff content and the number of lines detected in a diff according to the heuristics used for determining less important commits. The details on this categorization is also further explained in \autoref{sec:Heuristics}. \autoref{fig:Intelligent-History-B} shows an example of the metadata pop-up invoked for a selected commit.
...
\begin{figure}[hp]
    ...
    \caption{
        \FIXME{Make it clear (a) is the popup and (b) is the diff.} Feature (B) \textit{Show Diff Metadata} invoked on commit \commit{c3475081}.\textsuperscript{*}
    }%
...
\FIXME{Is it the reg ex or is the fact that heuristics are based on empirical observation?} While using regular expressions sacrifices some degree of accuracy on what is considered a ``trivial'' line of code change to a user,
...
\FIXME{Explain what motivated each choice.}
