...
\begin{figure}
    ...
    \caption{
        Three Jira issues opened in individual browser windows (left) for three commits from the file commit history (right) of the \code{Topology} class in Apache Kafka. Illustrates the distributed nature of code change and rationale information between the \entity{IDE} and browser.
    }
...
\begin{itemize}
    ...
    \item \textit{Import}: Changes which involve the insertion, deletion, or modification of \code{import} statements. We consider changes related to import statements to be involved in refactoring and unlikely to yield any further insight.
...
Overall, this reduces the time it would take for a developer to manually determine relevant commits and minimizes the switches the developer would need to make between their \entity{IDE} and browser to find and read the associated Jira issue.
