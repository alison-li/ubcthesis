...
The need to navigate a sizable quantiy of commits in a revision history 
...
Additionally, developers demonstrate a need for more convenient access
...
Our approach involves analyzing the diffs between commits in a file's commit history to determine which commits may be less significant in terms of the lines of code changed in the diff.
...
we devise four categories of changes to use for identifying less important commits based \rev{on} their diff content.
...
This is because the information Intelligent History offers is integrated within the \entity{IDE}, 
...
\begin{figure}
    ...
    \caption{
        A \rev{screen} capture of IntelliJ IDEA. 
...
\begin{itemize}
    ...
    \item[(\feature{3})] \textit{Show Jira Issue}: 
        ...
        this action also provides a metadata summary of the Jira issue containing metrics that could indicate an issue as likely to contain rationale information as an
...
For the commit history of \rev{the file \file{StreamsBuilder.java}, another Java class from the Apache Kafka project 
...
\rev{Similar to Kawrykow and Robillard \cite{kawrykow_non-essential_2011}, 
...
Represented as regular expression patterns for the Java programming language \rev{(see Appendix \ref{sec:Reg-Ex-Patterns})}, 
...
\entity{KIPs} contain the following sections: Motivation, Proposed Change, New or Changed Public Interfaces, Migration Plan and Compatibility, and Rejected Alternatives.} 
stating the purpose of the change was to address the overall accumulation of heavy method overloading in the 
